...
Necesidad del negocio: Realizar las pruebas automatizadas al modulo Login
  ...
  Esquema del escenario: Verificar si un usuario podra iniciar sesion con info invalida
    Cuando el ingresa "<username>", "<password>", "<repeatpassword>" y "<description>"
    Y ingresa el "<email>"
    Entonces valida el las alertas para cada campo "<description>"

    Ejemplos:
      | username         | password          | repeatpassword    | email                           | description                     |
    # Validation username
      | ionixtester      | ValidPassword#0   | ValidPassword#0   | abel.gutierrezarias@outlook.com | UN - Usuario Exitente           |
      | @invalidUserName | ValidPassword#0   | ValidPassword#0   | abel.gutierrezarias@outlook.com | UN - Usiario Invalido           |
